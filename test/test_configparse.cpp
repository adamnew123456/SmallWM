...
int main()
{
    ...
    if (class_c.actions & ACT_SETLAYER)
        std::cout << "[actions.class-c] Does includes a SETLAYER action even with a wrong layer\n";

    if (class_c.snap != SNAP_TOP)
        std::cout << "[actions.class-c] Does not snap top\n";

    if (config.key_commands.snap_top != XK_Up)
        std::cout << "[keyboard.snap-top] Default not set correctly.\n";

    if (config.key_commands.request_close != XK_asciitilde)
        std::cout << "[keyboard.request-close] New value not read correctly.\n";

    if (config.key_commands.iconify != XK_semicolon)
        std::cout << "[keyboard.iconify] New value not read correctly.\n";

    if (config.key_commands.force_close != XK_x)
        std::cout << "[keyboard.force-close] Did not revert to default when using incorrect value.\n";
}
